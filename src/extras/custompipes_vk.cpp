...
rw::vk::Shader *neoVehicleShader;

static void
uploadSpecLights(void)
{
	using namespace rw::vk;
...
static void
vehicleRenderCB(rw::Atomic *atomic, rw::vk::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::vk;
...
#include "shaders/obj/neoVehicle_vert.inc"
	...
	rw::vk::ObjPipeline *pipe = rw::vk::ObjPipeline::create();
	pipe->instanceCB = rw::vk::defaultInstanceCB;
...
DestroyVehiclePipe(void)
{
	...
	((rw::vk::ObjPipeline*)vehiclePipe)->destroy();
	vehiclePipe = nil;
}
...
rw::vk::Shader *neoWorldShader;

static void
worldRenderCB(rw::Atomic *atomic, rw::vk::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::vk;

	if(!LightmapEnable){
		vk::defaultRenderCB(atomic, header);
		return;
	}
...
DestroyWorldPipe(void)
{
	...
	((rw::vk::ObjPipeline*)worldPipe)->destroy();
	worldPipe = nil;
}
...
rw::vk::Shader *neoGlossShader;

static void
glossRenderCB(rw::Atomic *atomic, rw::vk::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::vk;
...
DestroyGlossPipe(void)
{
	...
	((rw::vk::ObjPipeline*)glossPipe)->destroy();
	glossPipe = nil;
}
...
rw::vk::Shader *neoRimShader;
rw::vk::Shader *neoRimSkinShader;
...
static void
rimSkinRenderCB(rw::Atomic *atomic, rw::vk::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::vk;

	if(!RimlightEnable){
		vk::skinRenderCB(atomic, header);
		return;
	}
...
static void
rimRenderCB(rw::Atomic *atomic, rw::vk::InstanceDataHeader *header)
{
	using namespace rw;
	using namespace rw::vk;
...
#include "shaders/obj/neoRim_vert.inc"
	...
	pipe = rw::vk::ObjPipeline::create();
	pipe->instanceCB = rw::vk::skinInstanceCB;
	pipe->uninstanceCB = nil;
...
DestroyRimLightPipes(void)
{
	...
	((rw::vk::ObjPipeline*)rimPipe)->destroy();
	rimPipe = nil;

	((rw::vk::ObjPipeline*)rimSkinPipe)->destroy();
	rimSkinPipe = nil;
}
...
{
	u_viewVec = rw::vk::registerUniform("u_viewVec");
	u_rampStart = rw::vk::registerUniform("u_rampStart");
	u_rampEnd = rw::vk::registerUniform("u_rampEnd");
	u_rimData = rw::vk::registerUniform("u_rimData");

	u_lightMap = rw::vk::registerUniform("u_lightMap");

	u_eye = rw::vk::registerUniform("u_eye");
	u_reflProps = rw::vk::registerUniform("u_reflProps");
	u_specDir = rw::vk::registerUniform("u_specDir");
	u_specColor = rw::vk::registerUniform("u_specColor");
}
...
{
	rw::Matrix matrix;
	rw::vk::InstanceDataHeader *instHeader;
	uint32 cullMode;
	uint8 fadeAlpha;
	bool lighting;
};
...
IsTextureTransparent(RwTexture *tex)
{
	if(tex == nil || tex->raster == nil)
		return false;
	return PLUGINOFFSET(rw::vk::Gl3Raster, tex->raster, rw::vk::nativeRasterOffset)->hasAlpha;
}
...
AtomicFirstPass(RpAtomic *atomic, int pass)
{
	...
	building->instHeader = (vk::InstanceDataHeader*)atomic->geometry->instData;
